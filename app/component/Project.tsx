...
const Project: React.FC<ProjectProps> = ({ project, index, setModal, isLast }) => {
    ...
    return (
        <a href={href} target="_blank" className={`group flex w-full items-center justify-between text-white py-12 px-24 border-t border-solid border-slate-500 cursor-pointer ${isLast ? 'border-b border-solid border-slate-500' : ''}`} onMouseEnter={() => setModal({isActive: true, project: index})} onMouseLeave={() => setModal({isActive: false, project: index})}>
